...
function getStaff() {
    fetch(API_URL)
        .then(response => {
            ...
            <tr>
                <td colspan="6" class="error-message">Faled to load staff data.</td>
            </tr>`;
        });
}
...
function renderStaff(data) {
    ...
    if (data.length === 0) {
        staffTableBody.innerHTML = `
            <tr>
                <td colspan="6" class="empty-message">No staff found.</td>
            </tr>`;
        return;
    }
    ...
    data.forEach((staff, index) => {
        ...
        row.innerHTML = `
            <td class="staff-number">${index + 1}</td>
                <td><img src="${staff.avatar}" alt="${staff.name}" class="staff-avatar"></td>
                <td><span class="staff-name">${staff.name}</span>
            </td>
            <td>
                <span class="gender-badge">${staff.gender}</span>
            </td>
            <td>${formattedBirthday}</td>
            <td>
                <div class="action-buttons">
                    <button type="button" class="btn-edit" onclick="openEditModal('${staff.id}')">Edit</button>
                    <button type="button" class="btn-delete" onclick="openDeleteModal('${staff.id}', '${escapeAttribute(staff.name)}')">Delete</button>
                </div>
            </td>`;
...
function openEditModal(id) {
    ...
    fetch(`${API_URL}/${id}`)
        .then(response => {
            if (!response.ok) throw new Error("Cannot fetch staff information.");
            return response.json();
        }).then(data => {
...
staffForm.addEventListener("submit", function(e) {
    ...
    const isoBirthday = selectedDate
    ...
    if (currentMode === "add") addStaff(staffData);
    else if (currentMode === "edit") updateStaff(currentStaffId , staffData);
})
...
function addStaff(staffData) {
    ...
    fetch(API_URL, {
        ...
        alert("Staff succesffully added!");
...
function deleteStaff(id) {
    ...
    fetch(`${API_URL}/${id}`, {
        ...
        /* DELETE APIs sometimes return an empty response (204).
         * Therefore we don't call response.json() here. */
        return;
    }).then(() => {
...
btnClose.addEventListener("click", () => closeModal(staffModal));
btnCancel.addEventListener("click", () => closeModal(staffModal));
btnDeleteCancel.addEventListener("click", () => closeModal(deleteModal));
btnDeleteConfirm.addEventListener("click", () => {if (staffToDeleteId !== null) deleteStaff(staffToDeleteId);});

/* CLOSE WHEN CLICKING OUTSIDE MODAL */
staffModal.addEventListener("click", function(e) {if (e.target === staffModal) closeModal(staffModal);});
